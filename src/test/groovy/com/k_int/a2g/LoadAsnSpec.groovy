...
class LoadAsnSpec extends Specification {
  ...
  def "Test decoding and encoding of Z3950 APDUs"() {
    when:
      ...
      byte[] init_request_ber = IOUtils.toByteArray(this.getClass().getResourceAsStream('/apdus/init_request.raw'));
      byte[] init_response_ber = IOUtils.toByteArray(this.getClass().getResourceAsStream('/apdus/init_request.raw'))
      byte[] search_request_ber = IOUtils.toByteArray(this.getClass().getResourceAsStream('/apdus/search_request.raw'))
      byte[] search_response_ber = IOUtils.toByteArray(this.getClass().getResourceAsStream('/apdus/search_response.raw'))
      byte[] present_request_ber = IOUtils.toByteArray(this.getClass().getResourceAsStream('/apdus/present_request.raw'))
      byte[] present_response_ber = IOUtils.toByteArray(this.getClass().getResourceAsStream('/apdus/present_response.raw'))
    ...
    then:
      ...
      BaseDecoder ber_decoder = new BERDecoder(new ByteArrayInputStream(init_request_ber))

      // Decode the init request file
      def decoded_init_request = asn_codec.decode('Z39-50-APDU-1995','PDU',ber_decoder);

      // Re-encode the decoded message
      def encoded_init_request = (byte[]) [] // asn_codec.ber('Z39-50-APDU-1995',decoded_init_request);
